...
export const updateDisplay = (weatherJson, locationObj) => {
    ...
    const ccArray = createCurrentConditionsDivs(weatherJson);
...
const createCurrentConditionsDivs = (weatherObj) => {
    ...
    const wind = createElem(
        "div", 
        "wind", 
        `Wind ${Math.round(Number(weatherObj.current.wind_speed))} m/s`
    );
    const hour1 = createElem(
        "div",
        "hour1", 
        `1 hour - ${weatherObj.hourly[1].temp}°`
    );
    const hour2 = createElem(
        "div",
        "hour2", 
        `2 hour - ${weatherObj.hourly[2].temp}°`
    );
    return [icon, temp, desc, feels, maxTemp, minTemp, humidity, wind, hour1, hour2] ;
};
...
const translateIconToFontAwesome = (icon) => {
    ...
    switch (firstTwoChars) {
        ...
        case "03":
            ...
            break;
        case "04":
            i.classList.add("fas", "fa-cloud-meetball");
            break;   
        case "09":
